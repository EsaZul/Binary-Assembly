...
;Checks the LIST
;********************************************************************************************************************
CHK	LDR	R4, R0, #1	;Loads the next PTR	
...
MATCH	ADD	R0, R0, #-4	;Deletes the effects of the counter, has original PTR of the matched list
	...
	STR	R2, R0, #1	;Stores the PTR found in 3800 into the 2nd memory of new list(WILL CONTAIN A NULL or POINTER AUTO);changed the offset
	ADD	R2, R2, #0	;Recalls R2
	BRZ	NOLIST
	STR	R0, R2, #0	;Stores previous last PTR in the second adrress of the one before it; changed the offset
NOLIST	STR	R0, R1, #0	;Puts PTR of the List at x3800 aka 306 PTR
	STR	R7, R0, #0	;Puts Null in the Second Space of the Moved list; changed the offset
